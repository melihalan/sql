...
SELECT c.customer_id, c.customer_first_name, c.customer_last_name, sum(cp.quantity * cp.cost_to_customer_per_qty) as total_spent_amount

FROM customer c INNER JOIN customer_purchases cp ON c.customer_id = cp.customer_id

GROUP BY c.customer_id

HAVING total_spent_amount > 2000;
...
SELECT customer_id, strftime('%m',market_date) as purchase_month, strftime('%Y',market_date) as purchase_year, sum(quantity * cost_to_customer_per_qty) as total_spent_amount
FROM customer_purchases
